...
export class UserEpinService {
  ...
  async getById(
    ...
    const userEPins = await UserEpin.getByUserId(
      userId,
      status,
    );
    return userEPins.map(e => e.responseObject);
  }

  async sendToAnother(userId: string, data: SendEPinDTO) {
    const { sendTo, total } = data;
    const availableEPins = await UserEpin.getByUserId(userId);

    if (availableEPins.length < total) {
      throw new HttpException(
        `Not enought epins. Available E-Pins ${availableEPins.length}`,
  ...
  async activateAccount(ownerId: string, userId: string) {
    const availableEPins = await UserEpin.getByUserId(ownerId);

    if (availableEPins.length < 1) {
