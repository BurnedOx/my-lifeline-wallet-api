...
export class UserEpinService {
    ...
    async redeemEpin(count: number, userId: string) {
        ...
        const totalCost = count * (EPIN_PRICE + (EPIN_PRICE * 0.10));
        const epins: EPin[] = [];
        ...
        await getManager().transaction(async trx => {
            for (let i = 0; i < count; i++) {
                ...
                await trx.save(userEpin);
                const history = this.historyService.createHistory(owner, epin, 'Redeemed from wallet money');
                await trx.save(history);
                epins.push(epin);
            }
    ...
    public async purchaseEpins(userId: string, count: number) {
        ...
        await getManager().transaction(async trx => {
            for (let i = 0; i < count; i++) {
                ...
                const userEpin = UserEpin.create({ owner, epin });
                await trx.save(userEpin);
                const history = this.historyService.createHistory(owner, epin, 'Purchased');
                await trx.save(history);
            }
        });
    }
